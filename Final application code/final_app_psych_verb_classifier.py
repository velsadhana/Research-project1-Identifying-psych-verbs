...
def tokenize(sentence):
    ...
    tkens = tokenizer(sentence,max_length=max_len,pad_to_max_length=True, truncation=True)
    #inputids=tkens["input_ids"]
...
def get_verb_pos(verb_count,token_id,seq_pos,token_words,pn_list,pn1_list):
    ...
    N=5
    if len(actual_pos)<5:
        actual_pos = actual_pos + [98] * (N - len(actual_pos))
    else:
         actual_pos
    return actual_pos
...
lbl3=tk.Label(master=frame1, text="Psych verbs identifier",bg="#CCD1D1",fg="blue",font=('Calibri',9,'bold'))
